...
:- object(connections(_Pkg_)).
   ...
   shift(
     c(StS-StD  ,S-D,sa(SS-AS,  SD-AD  ),List),
     c(StS1-StD1,S-D,sa(SS1-AS1,SD1-AD1),List1),
     backward(Event)
   ):-
     state(_Pkg_)::shift(
       c(StD-StS  ,D-S,sa(SD-AD  ,SS-AS  ),List),
       c(StD1-StS1,D-S,sa(SD1-AD1,SS1-AS1),List1),
       Event
   ),!.
   shift([State|T],T, closed(AD)) :-
